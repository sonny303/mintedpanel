...
//   TS-28 all-complete state: the CTA hands off to the remaining preview
//         (Scope Review since E1.5 activated Payer Network)
...
test("TS-25: fresh org shows the full journey — derived chips and disabled previews", async ({
  ...
  // E1.5 activated Payer Network: with no curated shortlist it renders the
  // honest no-payers-enabled empty state, not a disabled preview.
...
test("TS-28: every active section complete hands off to the Scope Review preview", async ({
  ...
  const fixtures = makeFixtures({
    ...
    // E1.5: ...and ≥1 active payer attachment target.
    payers: [
      {
        id: "pay-1",
        org_id: null,
