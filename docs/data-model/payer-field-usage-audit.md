...
Traced against `origin/redesign` @ `cfbfa405`. Live schema + data inventoried via Supabase MCP
...
| `emailTemplate` (`{subject,body}`)             | wizard (draft_email only)                       | `:110–116` interpolated                  | **Gmail handoff `CaseWizard.tsx:60` → `planGmailHandoff` (`gmailCompose.ts:24`)**                                   | 1                       |
