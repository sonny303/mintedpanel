...
| provider_facility_assignments | party     | core                                           | 4                       | template for planned `provider_group_assignments` (provider↔group is really M:N); `provider_id`/`facility_id` NOT-NULL + single `is_primary` per provider enforced (E0.10)                     |
| state_licenses                | party     | core                                           | 4                       | source of truth for licensure; `provider_id` NOT-NULL + `state` format enforced (E0.10)                                                                                                        |
