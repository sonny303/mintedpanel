...
| provider_facility_assignments | party     | core                                           | 4                       | template for `provider_group_assignments` (built E1.3); `provider_id`/`facility_id` NOT-NULL + single `is_primary` per provider enforced (E0.10)                                               |
...
| credential_cases              | work      | **core (center)**                              | 7                       | unique `(provider_id, payer_id, state)`; `state` format `^[A-Z]{2}$` enforced (E0.10); `credentialing_status_id` nullable (MVD gap — explicit E0.10 non-goal)                                  |
...
| tasks                         | work      | core                                           | 5                       | owner CHECK enforced (E0.10): a task must carry `case_id` or `provider_id`                                                                                                                     |
