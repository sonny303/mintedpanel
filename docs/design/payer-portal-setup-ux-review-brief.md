...
- **No backend redesign.** Tables, RPCs, and `/api` contracts stay as-is unless you flag a *tiny* additive UI need on an **already existing** write path (e.g. `updatePortalUrl` already exists in code but is not reachable from the live Form setup UI).
...
- **Preserve domain grains** described in §4 (case key, global catalog, portal key immutability, template publish model). Redesign *how people reach and maintain* those objects — not what the objects *are*.
...
| **Delete / retire a portal** | **No product UI** for delete or soft-retire. Wrong keys, duplicates, and abandoned registrations linger in pickers (ghost filtering only drops portals whose *payer* is retired/merged/archived — not “I registered the wrong URL”). |
...
2. **One published checklist story per payer** is the readiness spine; form readiness is a *secondary* ladder for portal paths (a no-online-form SOP can still be “ready” with a note).
