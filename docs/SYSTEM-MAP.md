# Minted Panel system map

The detailed orientation that used to live in `CLAUDE.md`. It is no longer
auto-loaded into every session — read it when you need the system map, the
schema/hosted-drift rules, the `/api` layer contracts, or the history of an
epic. `CLAUDE.md` is now a short index that points here. **Keep this file
current at session end** (the ritual at the bottom applies to this file now).
