...
| **R6 — Execution**           | E4.0 payer pipeline (tracking IDs, structured resolution + denial reasons), E4.1 structured touches + follow-up cadence, E4.2 Payer & SOP admin module (upstream config, bulk generation entry), E4.3 extension workbench handoff (read-only fill), E4.4 Sensitive Identifiers Vault (SSN), E4.5 Document Storage (provider + group docs, **mandatory expiration tracking** — the E1.8 deferred surface)                                                                                                                                                                                                | 🚧 Discovery resolved ([r6], 2026-07-14, incl. NBA-queue configurability: org-level admin-configurable with default); six epics drafted — review next.                                                                                                                     |
| **R7 — Execution follow-on** | Reserved for R6 spillover / deferred execution scope (e.g. email-inbox↔touch integration with auto-created Email touches + human-confirmed suggested RFI transitions, standing per-payer cadence rules; **payer-contact directory/CRM** — promote the E4.1 per-touch recipient/contact fields to payer-level contact records once repeated touches establish confidence in who to contact, PM 2026-07-14; **global settings + role/capability administration** — a global/cross-org defaults layer that orgs inherit/override (E4.2 F4.2.5 ships org-level only) plus a richer role model beyond `admin | specialist                                                                                                                                                                                                                                                                 | billing` (config-only role, per-role permissions), PM 2026-07-14) | 📋  |
...
2. **PM manual test pass** — the merged R5 import flow (intake → preview/staged commit → sectioned uniformity) on the `redesign` preview.
