...
**2026-07-13** (R4 epics published)._

```
R0 ✅ ──► R1 ✅ ──► R2 ✅ ──► R3 ✅ ──► R4 🚧 epics published, review queue ──► R5…R10 📋
```
...
| **R4 — Case generation**    | E1.7b SOP-as-data, E2.x preview/generate/dedupe/traceability/audit          | 🚧 All six epics PM-approved & published (#132): E1.7b SOP-as-data + E2.0 preview / E2.1 creation & 4-part key / E2.2 SOP stamping / E2.3 next-best-action queue / E2.4 traceability. Now queued for independent Claude review, then sequential builds E1.7b → E2.0 → … → E2.4. |
...
| ----- | ------------------------------- | -------- | ----- |
| E1.7b | SOP-as-data (Model A build)     | ⏳ queue | —     |
| E2.0  | Generation preview & exclusions | ⏳ queue | —     |
| E2.1  | Case creation & 4-part key      | ⏳ queue | —     |
| E2.2  | SOP resolution & stamping       | ⏳ queue | —     |
| E2.3  | Next-best-action queue          | ⏳ queue | —     |
| E2.4  | Generation traceability & audit | ⏳ queue | —     |
...
1. **R4 review queue** — six Claude Code review sessions (E1.7b first), one per epic per `REVIEW-HANDOFF.md`; PM approval flips `reviewed: true`, then sequential builds.
