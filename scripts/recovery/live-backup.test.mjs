...
for (const cleanupMode of ["exact-role", "exclusive-staging-maintenance"]) {
  test(`coordinator passes ${cleanupMode} and persists only sanitized evidence`, async () => {
    const calls = [];
    let saved;
    const result = await captureLiveStagingBackup(
      { workspace: "/private/tmp/recovery", recipient, cleanupMode },
      {
        loadToken: async () => {
          calls.push("token");
        ...
        withLogin: async ({ token, operation, cleanupMode: actualMode }) => {
          assert.equal(actualMode, cleanupMode);
          assert.equal(token, "private-token");
          ...
          return {
            ...
            lifecycle: {
              ...
              cleanupRequestedAt: "2026-09-19T04:00:03.000Z",
              cleanupReceivedAt: "2026-09-19T04:00:03.100Z",
