...
export const GROUP_DESCRIPTOR: SectionDescriptor = {
  ...
  buildMapped(ctx) {
    ...
    return {
      ...
      website_url: ctx.nullable("website_url"),
      ...billing,
...
export const PROVIDER_DESCRIPTOR: SectionDescriptor = {
  ...
  helperText:
    "Re-download the current template; old versions are rejected. One row per provider × license (repeat identity columns for extra licenses, facilities, groups, or enrollments). Contact, education, start date, new-grad/CAQH, and license type fields are optional. Copy facility_name from the reference sheet to assign an existing location — the first facility on the file is the primary. Only the last 4 SSN digits in ssn_last4. The parent group is matched by group_tin then group_name.",
  spec: {
