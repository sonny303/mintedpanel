...
// extension, there is no idempotency key from a content script, and the fill
// happens entirely in this tab. It is the same browser RLS insert the test
// runner uses, with the case attached, `is_test` false, and `fill_mode='pdf'`.
//
// Only counts and per-field reasons are stored. No field VALUE is written to
// the row or the audit trail — the values went into a local PDF and stay there.
// ---------------------------------------------------------------------------
...
export async function recordPayerFormFill(input: PayerFormFillInput): Promise<FillSession> {
  ...
  await writeAudit({
    ...
    description: `Payer form filled (${session.portalKey})`,
  });
  return session;
}
