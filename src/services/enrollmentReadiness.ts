...
export async function listProviderReadinessFacts(): Promise<ProviderReadinessFacts[]> {
  ...
  const { data, error } = await supabase
    .from("providers")
    .select(
      "id, first_name, last_name, status, verification_state, npi, caqh_id, caqh_last_attested_date, date_of_birth, ssn_last4, home_street, home_city, home_state, home_zip",
    )
    .eq("org_id", orgId)
