...
describe("createDocumentUploadIntent", () => {
  ...
  // TS-164/165 — a gap upload from the step's required-documents panel sends
  // the CANONICAL kind with the case as usage context. The case dimension is
  // authorized at INTENT time, not just finalize, so a signed target is never
  // minted for a case the caller cannot see.
  it("404s an intent whose caseId is cross-org, before minting a signed target", async () => {
    const { db, calls } = fakeDb({
  ...
  it("404s an intent whose caseId is org-owned but belongs to another provider", async () => {
    const { db, calls } = fakeDb({
      providers: [orgProvider],
      credential_cases: [
  ...
  it("mints the intent when the caseId links to the owner", async () => {
    const { db, calls } = fakeDb({
      providers: [orgProvider],
  ...
  it("422s filled_form on either grain — it stays dormant, never step-uploadable", async () => {
    const { db, calls } = fakeDb({ providers: [orgProvider] });
    const result = await createDocumentUploadIntent(ctx(db), {
      ...intentInput,
      kind: "filled_form",
