...
// the TABBED, editable detail. Un-nested from the setup segment (the
// `setup_` idiom, like generation_.runs) so it renders as its own page.
// Slice G renamed the segment catalog → setup; /admin/payer-admin/setup/
// $payerId still redirects here carrying ?tab= and ?edit=.
...
// Both params are OPTIONAL: every existing link to this payer (Payer Setup,
// the case header, the near-match "use this one" hand-off) navigates without
// search, and a bare URL must keep working. The route normalizes — an unknown
// tab reads as Overview.
interface PayerDetailSearch {
  tab?: PayerDetailTab;
  edit?: boolean;
}

export const Route = createFileRoute("/admin/payer-admin/setup_/$payerId")({
