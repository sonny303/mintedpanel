...
import { useCreateSop, usePayers, useSop, useUpdateSop } from "@/hooks/useAdmin";
...
function TemplateEditor() {
  ...
  const updateMut = useUpdateSop(id);
  ...
  async function handleToggleArchive() {
    ...
    try {
      ...
      toast.success(next ? "SOP archived" : "SOP restored");
    } catch (err) {
      setIsArchived(!next);
  ...
  if (!tpl) {
    return (
      <div className="p-6">
        <PageHeader title="SOP not found" />
      </div>
    );
  }
            ...
            {tasks.length === 0 ? (
              <div className="rounded-md border border-dashed border-[#E8E5E0] p-6">
                <EmptyState
                  ...
                  description="Add a task to start building this SOP"
                />
              </div>
            ) : null}
