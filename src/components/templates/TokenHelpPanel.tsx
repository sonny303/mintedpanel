    ...
    <aside className="lg:sticky lg:top-6 self-start">
      <div className="rounded-md border border-[#E8E5E0] bg-card">
            ...
            previewTasks.map((t, i) => (
              <div key={i} className="space-y-1">
                ...
                <ol className="space-y-2 mt-2">
                  {(t.steps ?? []).map((s, j) => {
                    const fields = (s as { dataFields?: DataField[] }).dataFields ?? [];
                    return (
        ...
        <div className="px-4 py-3 border-t border-[#E8E5E0] text-xs text-muted-foreground">
          ...
          <span className="font-mono">{sampleValues["provider.lastName"]}</span>, NPI{" "}
          <span className="font-mono">{sampleValues["provider.npi"]}</span>.
        </div>
      </div>
    </aside>
  );
