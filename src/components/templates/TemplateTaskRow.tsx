...
import { ALL_DOCUMENT_KINDS, DOCUMENT_KIND_META, parseDocumentKind } from "@/lib/documents";
...
//
// `filled_form` is excluded (dormant, never uploadable) and so is `other`,
// which is a valid vault kind but carries no meaning as a payer requirement —
// offering it would reintroduce exactly the ambiguity the picker removes.
//
// Legacy free-text entries authored before this picker are shown, editable,
// and never rewritten or dropped (US-1 AC3); at case time they render as
// inert notes rather than a false "Missing" (stepDocumentRequirements).
const REQUIREABLE_KINDS = ALL_DOCUMENT_KINDS.filter(
  (k) => DOCUMENT_KIND_META[k].uploadable && k !== "other",
);
