// Fix-it deck (Surface 1), extracted from the retired /fix-it route by E6.1
// F6.1.6 and mounted as the Payer Setup workspace's "Needs attention" tab —
// the interim home until E6.5's drift-repair-in-the-editor supersedes the
// deck (F6.5.4 "The Fix-it deck retires"). Behavior unchanged: a deck of
// 30-second decisions ordered by soonest blocked fill — never by ease; no
// timers, no speed mechanics; corrections are celebrated as good catches.
// Training hand-offs land on the Forms & portals tab (the /portals/$key/train
// deck retired with this restructure; E6.5 rebuilds training in the step
// editor).
...
export function FixitDeck() {
  ...
  // The retired training deck's replacement home (E6.5 rebuilds training in
  // the SOP form step editor).
  const openTraining = () => navigate({ to: "/admin/payer-admin", search: { tab: "forms" } });

  if (!canWrite) {
    return (
      <div className="py-16">
        <EmptyState message="The needs-attention deck is available to specialists and admins." />
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-[12.5px] text-muted-foreground">
          Each card is one decision. Clearing it improves tomorrow&apos;s fills.
        </p>
        {catchChip}
      </div>
      <div className="mx-auto max-w-[600px]">
        {s.deck.length === 0 ? (
          ...
          <SessionSummary session={s} onDone={() => navigate({ to: "/cases" })} />
        ) : (
          <>
    ...
    <CardShell
      ...
      footer={
        <>
          ...
          <span className="ml-auto text-[11.5px] text-[color:var(--mp-ink-faint)]">
            training moves into the SOP editor with E6.5
          </span>
        </>
      }
    >
...
function SessionSummary({ session, onDone }: { session: Session; onDone: () => void }) {
