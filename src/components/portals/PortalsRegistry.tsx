// The payer-portal registry (cleanup Surface 3): URLs, mapped/proposed/unlinked
// field counts, verification, last fill result. Extracted from the
// /admin/portals route (E4.2 unified payer setup, TE-19) so the standalone page
// and the Payer Setup workspace's "Forms & portals" tab compose ONE registry.
// Editing stays deliberately narrow — URL edits here, field decisions in
// training. `initialAddPayerId` lets a payer-context deep link (the setup
// funnel's "Register portal" action, /admin/portals?payerId=…) open the Add
// dialog with that payer preselected.
...
interface PortalRow {
  ...
  mapped: number;
  proposed: number;
  // Non-retired maps with a live selector but no token/hardcoded/manual source
  // — they stay blank on every fill until someone links them.
  unlinked: number;
...
export function PortalsRegistry({ initialAddPayerId }: { initialAddPayerId?: string | null }) {
  ...
  // A payer-context deep link opens the Add dialog immediately (admins only —
  // the control never renders for a role that couldn't complete it).
  const [adding, setAdding] = useState(Boolean(initialAddPayerId));
  ...
  const rows: PortalRow[] = useMemo(() => {
    ...
    for (const m of maps) {
      ...
      if (isUnlinkedFieldMap(m))
        unlinkedByKey.set(m.portalKey, (unlinkedByKey.get(m.portalKey) ?? 0) + 1);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start gap-3">
        <div className="flex-1 border border-[#E8E5E0] rounded-md bg-[#FAFAF9] px-4 py-3 text-[13px] text-foreground">
          Field selectors are captured by the extension and approved in training. This screen
          manages where they point and whether they're trusted.
        </div>
        {isAdmin ? (
          <Button
      ...
      {adding ? (
        <AddPortalModal
          initialPayerId={initialAddPayerId ?? null}
