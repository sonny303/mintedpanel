...
`id, org_id, name, is_active, avg_decision_days, created_at`. **`org_id` is NULLABLE (P2, migration `20260707060000`): a NULL row is a global-catalog payer, visible to an org only via `org_payer_assignments`.** SELECT policy = `(org_id IN user_org_ids()) OR (org_id IS NULL AND assigned)`; **org users can't write payers at all since `20260718120000_payers_org_write_lockdown` (payers_insert/payers_update dropped, INSERT/UPDATE grants revoked — member SELECT only; catalog writes are service-role).** **E1.6 catalog identity columns (`20260712180000`):** `payer_kind` (CHECK commercial|medicare|medicaid|medicaid_mco|medicare_advantage|tricare, default commercial), `status` (active|merged|retired), `aliases text[]`, `states text[]`, **`payer_slug`** (canonical dataset key, partial UNIQUE `uq_payers_payer_slug WHERE payer_slug IS NOT NULL` — the sync dedupe/identity key), dormant `merged_into_id` self-FK, `last_synced_at`. The dead curated-billing block plus `cms_hios_id`/`prerequisite_payer_id` were dropped in the pre-GA cleanup (`20260717221914`, superseding the never-applied `20260716180000`). **E4.2 governance (+ 2026-07-18 close-out):** the app has NO payers write path at all — no INSERT (canonical identities are selected from the catalog) and, since the close-out, no UPDATE either (`updatePayer` deleted; `payers.ts` is read-only). `resolution_id_label`/`resolution_id_expected` (E4.2 F4.2.1) are the Minted-curated GLOBAL fallback — the org-varying override lives in `org_payer_settings`. Legacy org-scoped rows are GONE (pre-prod-cut wipe 2026-07-17; pre-wipe data in `mintedpanel-backup-july17`) — the table is global-catalog-only and the cutover record (`docs/data-model/legacy-payer-cutover.md`) is closed as superseded. **E6.5 (`20260719170000`, repo-only):** additive `delegation_note text` — the curated delegation fact ("this payer delegates credentialing to X — submit via Y") that REPLACES the retired MSO routing engine app-side; rendered on the catalog browse. Rides `list_global_payers` (RETURNS SETOF payers) with no RPC change. **E6.7 — the manual-setup enabler (`20260727120000`, repo + hosted):** the table has ONE app write path again — the **`create_payer` / `update_payer` SECURITY DEFINER RPCs** (authenticated, anon rejected in-body, writer-member of the caller org, audited under that org; the `author_global_sop` interim posture, R7 hardens). Direct table DML stays revoked (the 20260718120000 lockdown stands — no policies, no grants; the RPCs are the only door). `create_payer` validates name (required), `payer_kind` (6-value domain), `states[]` (REQUIRED ≥1, `^[A-Z]{2}$`, normalized/deduped — attach eligibility, generation candidacy, and the attach-CSV scan all intersect against it), runs the duplicate guard (normalized name+aliases vs every non-retired global row's name/aliases → `payer_duplicate`; a merged match names its successor), inserts a GLOBAL row (`org_id NULL`, `status 'active'`, `payer_slug`/`last_synced_at` NULL) and upserts the caller org's `org_payer_assignments` row in the SAME transaction (creating = adding; reactivates an archived subscription). `update_payer` edits name/kind/states/aliases/ID pairs/delegation on ACTIVE global rows only (same validation + guard excluding self; states can never be emptied; status/merge/org/provenance NOT editable — merges stay platform-side). **Additive columns:** the ID-expectation split `group_id_label`/`group_id_expected` + `provider_id_label`/`provider_id_expected` (provider pair backfilled from the legacy `resolution_id_*` pair, which deprecates in place — stop-write; seam precedence provider pair → legacy pair → generic in `src/lib/payerResolutionIdentifier.ts`); provenance `created_by`/`source (seed|sync|manual` CHECK; pre-E6.7 rows backfilled `'sync')`/`updated_at`; partial unique **`uq_payers_global_normalized_name`** on `lower(btrim(name)) WHERE org_id IS NULL AND status <> 'merged'` (DB dup backstop; aliases stay RPC-guard-only; the RPCs convert its 23505 to `payer_duplicate`). `delegation_note`'s only writer is the RPC seam (`payerGovernance.test.ts` pins it). **E6.7 PR 2 (F6.7.4/F6.7.5, no schema change): the catalog sync is RETIRED** (`scripts/payer-catalog-sync.mjs` deleted; the reference dataset at `docs/redesign/data/payer-catalog/` is frozen) — `payer_slug` and `last_synced_at` are DEPRECATED IN PLACE (stop-write; seeded values retained per the additive rule), and stored `avg_decision_days` is no longer rendered anywhere (it lost its only writer with the sync; the column stays — TECH-DEBT TD-45 tracks the derive-from-case-outcomes replacement). **E6.8 — the lifecycle enabler (`20260727150000`/`150100`, repo + hosted):** additive `archived_at timestamptz NULL` — the REVERSIBLE org-workflow archive flag (deliberately not the platform `status` domain; the normalized-name dup backstop still covers archived rows). Set/cleared ONLY by the `archive_payer`/`reactivate_payer` SECURITY DEFINER RPCs (authenticated, anon rejected in-body, writer-member of the caller org, audited): archive rejects with `payer_archive_open_cases: <n>` while any org holds an open (non-terminal) case on the payer, writing nothing; exclusion from attach eligibility / generation candidates / the default payer-list derivation is client-side filtering on `archived_at IS NULL` (no RLS change — archived rows stay readable so history keeps resolving). `merge_payer(p_org_id, p_loser_id, p_survivor_id)` is the in-app duplicate repair: ONE transaction re-points `sop_templates` / `payer_network_targets` (survivor wins collisions, inherits the group PIN, loser duplicates archived in place) / `enrollment_facts` (colliding live loser facts expired first, PIN inherited) / OPEN `credential_cases` (a 4-part-key collision raises `payer_merge_case_conflict` listing case numbers, committing nothing) / `org_payer_assignments` (fold + dedupe), appends the loser's name to the survivor's `aliases[]`, and marks the loser `status 'merged'` + `merged_into_id`. Not undoable from the app; contacts/contracts/exclusions deliberately stay on the loser.

### payer_contacts (E6.7 F6.7.2a, 2026-07-27)

`id, payer_id → payers (CASCADE), purpose, name, email, phone, note, is_default, created_by, created_at, updated_at`. Operational contacts on a payer — a CHILD table at the purpose grain per this file's grain rule (purpose-varying facts are child rows, never columns on `payers`). `purpose` CHECK `credentialing|enrollment|escalation|general`; reachability CHECK (email OR phone — a contact you cannot reach is not a contact); partial `UNIQUE (payer_id, purpose) WHERE is_default` = ONE default per purpose, the future template email-step resolution target (steps will reference `(payer_id, purpose)` at resolution time — never a hard FK, so templates stay portable; the resolution seam is a later §5 decision, nothing resolves contacts in E6.7). RLS: member SELECT via the PARENT payer's visibility (the payers disjunct restated — own-org OR assigned-global; the `sop_template_versions` precedent); NO client write policies or grants (SELECT-only for authenticated, revoke-then-grant floor) — writes ride the audited **`upsert_payer_contact` / `delete_payer_contact` RPCs** (authenticated, writer-member, in-RPC default swap; hard delete allowed — operational data, not an append-only ledger). Migration `20260727120200` (repo + hosted). Service `src/services/payerContacts.ts` → `src/hooks/usePayerContacts.ts`; no rendered UI in E6.7.

### payer_forms (Payer PDF, 2026-08-24)

`id, template_id → sop_templates (SET NULL; nullable after owning template hard-delete), payer_id → payers, family_id, version, label, file_name, storage_path, mime_type, byte_size, supersedes_id → payer_forms, retired_at, retired_by, created_at, created_by`. The payer-specific blank PDF forms a template hands a coordinator (e.g. "PT Credentialing Supplement"). **GLOBAL rows: no `org_id`** — like the `sop_templates` the Template Editor authors and the shared portal/field-map catalog. A payer's blank form is the same file for every org credentialing with that payer and belongs to no provider, which is why it is NOT in the org-scoped, provider/group-owned `provider_documents` vault (that would have meant a fake owner and one copy per org). Migration `20260824170000_payer_forms.sql` (repo + hosted); FK detach + `delete_org_sop_template` in `20260903210000_delete_org_sop_template.sql`.

**Two grains, deliberately.** `family_id` is the stable identity a TEMPLATE ACTION points at (`task_definitions[].steps[].payerForm.familyId`); each upload under that family is an immutable row at `version`. A CASE task points at a concrete **row id**, baked at generation. That split is what makes a replaced file reach newly generated cases without republishing the template version, while a case already created keeps the exact file it was generated with. **"Current" is DERIVED** — highest live version in the family, never a flag (the `provider_documents` rule). Replace = INSERT a new version (`supersedes_id` → the prior row). Delete = soft `retired_at`; a retired family stops being attached to new cases but stays downloadable for cases that already point at it. `UNIQUE (family_id, version)` + `UNIQUE (storage_path)`; CHECKs on non-blank label/file_name, positive version/byte_size, and `mime_type = 'application/pdf'`.

**RLS/grants:** member SELECT `USING (true)` — a blank payer form is global truth carrying no tenant data, the same posture as the shared portal registry. There is NO client INSERT policy: uploads run through the guarded `/api/payer-forms/*` signing boundary on the service-role client (admin-only, see `src/server/payerFormRoutes.ts`). The ONE client write is the soft retire — `payer_forms_retire` (UPDATE, gated by the new SECURITY DEFINER helper **`user_is_admin_anywhere()`**, since a global row has no org for `user_role(org)` to be asked about) — narrowed further by the `payer_forms_retire_only_trg` BEFORE UPDATE trigger, which rejects a change to any column other than `retired_at`/`retired_by`. That is what keeps a "replace" an INSERT rather than an edit.

**Storage:** private bucket `payer-forms` (25 MiB, `application/pdf` only), path contract `payer/{payerId}/{familyId}/{version}/{safeFileName}` — no org segment, server-generated, never accepted from the browser. `payer_forms_objects_select` grants authenticated read; deliberately NO insert/update/delete policies (the service-role client behind the signing boundary is the only writer). Limits mirrored in `src/lib/payerForms.ts`.

### payer_catalog_changes (E1.6, 2026-07-12)

`id, payer_id, field, old_value, new_value, source, review_state (unreviewed|accepted|rejected), reviewed_by, reviewed_at, created_at`. Append-only sync diff log (migration `20260712180100`). **PLATFORM-ONLY since E4.2 governance (`20260716191000`):** the E1.6 authenticated shared-queue SELECT policy was dropped and ALL authenticated grants revoked; `review_payer_catalog_change` had authenticated EXECUTE revoked AND was reissued to reject org-user JWTs in the body (defense in depth — service_role and direct-SQL platform tooling are the only callers; `reviewed_by` is NULL for platform calls). Accept still applies the identity-field whitelist (name/aliases/states/status — never the slug or curated fields). The in-app review panel is gone; Minted reviews diffs via service-role/MCP tooling. `list_global_payers` (SECURITY DEFINER read) still serves the browse-everything directory without touching the P2 RLS disjunction. **DORMANT since E6.7 PR 2 (F6.7.4):** the catalog sync — the table's only diff writer — is retired, so nothing writes here anymore; table + review RPC kept per the additive rule.
...
**2026-07-22 (Cases page redesign) — Case#:** `case_number bigint NOT NULL` is a globally-sequential, immutable case number displayed as `C-<n>`. ONE sequence `credential_cases_case_number_seq` across ALL orgs (not per-org), so numbers reflect global creation order; backfilled in `created_at` order (base 1001), drawn by a column `DEFAULT nextval(...)` on every insert path (the `create_case_with_tasks` explicit column list is unchanged, so no RPC change was needed). Global `UNIQUE (case_number)`; a `BEFORE UPDATE` trigger (`credential_cases_case_number_immutable`) rejects any change to it. Sequence `USAGE` granted to `authenticated` (the SECURITY-INVOKER RPC draws the default) + `service_role`. Migration `20260722120000`, repo + hosted (applied via MCP the same day; the six live cases backfilled to 1001–1006, sequence next = 1007).
**E1.1 (2026-08-27) — `facility_id` is now the PRIMARY MIRROR of `case_facilities` (below):** the column's shape and every existing read path are unchanged — a case can now hold several locations via the child table, and this column always reflects whichever one is `is_primary = true` (or null when none are). Kept in lockstep by `addCaseFacility`/`removeCaseFacility`/`setPrimaryCaseFacility`, never written directly for a multi-location case outside those functions.

### case_facilities (E1.1, 2026-08-27 — multi-location cases)

`id, org_id, case_id → credential_cases, facility_id → facilities, is_primary, created_at, created_by → auth.users`.
The full location set for a case — additive alongside `credential_cases.facility_id`, which stays exactly as it is today (populated, still read the same way by every existing caller) and becomes the PRIMARY MIRROR of whichever `case_facilities` row has `is_primary = true`. `UNIQUE (case_id, facility_id)` (a case can't hold the same location twice); a partial unique index `uq_case_facilities_primary_per_case ON (case_id) WHERE is_primary` enforces at most one primary row per case (a CHECK can't see other rows, hence the index). Backfilled once, in the same migration, from every pre-existing case's `facility_id` (`is_primary = true`, `created_by NULL` = system backfill). RLS mirrors `credential_cases_select/insert/update` exactly (org-scoped SELECT; writer `admin|specialist` INSERT/UPDATE), plus DELETE for the same writer roles (the `provider_facility_assignments` shape — a location is removable, unlike a case itself). Eligibility is unchanged: a facility may only be attached when the provider is assigned to it under the case's group (`isEligibleCaseFacility`, `src/lib/caseFacility.ts`) — RLS can't cheaply express that join, so it's an app-level check in the service layer, same as `setCaseFacility` today. Migration `20260827120000_e11_case_facilities.sql`.

Service layer (`src/services/cases.ts`): `resolveCaseFacilityId` (creation-time stamp) and `setCaseFacility` (existing single-facility overwrite) are UNCHANGED — this table is purely additive/post-create. Three new writes, each keeping the mirror in lockstep in the same call: `addCaseFacility(caseId, facilityId)` (the case's FIRST location becomes primary + sets the mirror; every location after that is non-primary and the mirror is untouched), `removeCaseFacility(caseId, facilityId)` (removing the primary with other locations remaining auto-promotes the next by name — `pickNextPrimaryCaseFacility`, the same alphabetical sort `caseFacilityOptions` uses — and updates the mirror; removing the case's last location clears the mirror to null), `setPrimaryCaseFacility(caseId, facilityId)` (re-points to an EXISTING `case_facilities` row only — throws otherwise, this is not add-and-promote; demotes the old primary before promoting the target so the partial unique index is never transiently violated). All three are org-scoped, audited (before→after), and use the same `isEligibleCaseFacility` eligibility rule where a new attachment is involved. `getCaseFacilities(caseId)` returns the full set joined to `facilities`, sorted by name, for display.
...
**`set_case_status(p_case_id, p_to_status, p_expected_status, p_reason_code_id, p_note, p_is_correction, p_effective_date, p_individual_provider_id, p_group_provider_id, p_contract_executed_date, p_evidence_touch_id, p_provider_id_missing_ack, p_group_id_missing_ack) RETURNS jsonb`** (E6.0, migration `20260719120100`; reissued by E6.7 `20260727120000` and E6.8 `20260727150200` — the old 11-param signature is DROPPED so exactly one overload exists): the ONE atomic unified-status transition entry point. **SECURITY INVOKER**, pinned `search_path`, EXECUTE `authenticated` only. In one transaction it: validates the edge (spine-forward with skips + the action_required → in_review RFI return + any open → terminal + the denied → in_progress reapply; mirrored from `src/lib/caseStatus.ts` — a rejected edge RAISEs `case_status_invalid_transition` and rolls back, NO history row, no partial write), enforces the evidence rules (Approved REQUIRES `p_effective_date` and — **since the E6.7 `20260727120000` reissue — exactly the payer's EXPECTED IDs**: the individual ID when `payers.provider_id_expected` (falling back through the legacy `resolution_id_expected`, then TRUE = the pre-E6.7 rule) and the group ID when `payers.group_id_expected` (NULL → false), new named error `case_status_approved_needs_group_provider_id`; **since the E6.8 `20260727150200` reissue each expected ID may instead be explicitly acknowledged missing** (`p_provider_id_missing_ack`/`p_group_id_missing_ack`, default false — the per-ID "Didn't receive" escape; silence still raises the E6.7 errors, an acked ID stays NULL for the derived Awaiting-ID state, and a consumed ack is recorded in the audit `after` payload + a `Didn't receive: …` line in the history note); Denied REQUIRES a reason code, "Other" also the note; Not Pursuing REQUIRES the note), the admin-only correction gate (backward/off-edge moves, note required), optimistic concurrency (`p_expected_status` ≠ current → `case_status_conflict:<actual>`; the Add-touch bump passes NULL since an auto trigger may have just advanced the case), validates `p_evidence_touch_id` belongs to the same case, then updates `case_status` + the legacy mirrors (`credentialing_status_id` by canonical label lookup, `payer_pipeline_state` by fixed map), applies the terminal-fact writes (Approved: `confirmed_effective_date`, both payer IDs, `contract_executed_date`, `approved_date`; Submitted: `submitted_date` once; approval-reversal correction clears them), inserts the `case_status_history` row, and writes an in-RPC `audit_log` STATUS_CHANGE row.
...
### portals (2026-07-06; global tier E6.5)

`id, org_id, portal_key, name, payer_id, form_url, is_verified, last_verified_at, proven_at, url_changed_at, created_at, updated_at`. Registry of payer portals the extension can fill; unique `(org_id, portal_key)`. Editing `form_url` clears `is_verified` AND `proven_at` and stamps `url_changed_at` (drives the "Needs re-verify" pill). **E6.5 global tier (migration `20260719170000_e65_global_authoring.sql`, repo-only — hosted apply is an operator step):** `org_id` is now NULLABLE — a NULL row is a GLOBAL registry entry ("registered once, inherited by every org"), uniqueness backed by the partial `uq_portals_global_key (portal_key) WHERE org_id IS NULL`; the SELECT policy gained the global disjunct (`org_id IS NULL OR own-org`); writer INSERT/UPDATE policies stay own-org-only, so global writes go ONLY through the SECURITY DEFINER RPCs `upsert_global_portal` (key immutable after create; a form-URL change clears verification + proof) and `set_global_portal_flags` (verified/proven flips) — both REVOKE anon/PUBLIC, GRANT authenticated+service_role, and reject `anon` in-body via `auth.role()` (interim F6.5.6 governance: open to all authenticated users; R7 hardens). `proven_at` is the F6.5.3 mock-dry-run pass stamp (a dry run over the SYNTHETIC mock profile that resolves every decided mapping). Original migration `20260706120000_cleanup_surfaces_schema.sql`.
