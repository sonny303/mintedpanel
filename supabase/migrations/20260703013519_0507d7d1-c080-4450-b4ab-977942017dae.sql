-- B4a: atomic case creation. One transaction for credential_case +
-- initial status_history + audit_log + spawned tasks. SECURITY INVOKER
-- so all inserts go through the caller's RLS policies.
-- Applied to the dev project via MCP as "create_case_with_tasks_rpc".
--
-- p_input keys (snake_case, matching credential_cases columns):
--   org_id (required), provider_id, payer_id, state,
--   group_id, facility_id, specialty, credentialing_status_id (optional —
--   defaults to the org's first credentialing status by sort_order),
--   mso_id, assigned_to, submitted_date, expected_effective_date
-- p_tasks: jsonb array of {title, description, sop_content, sort_order, due_date}

CREATE OR REPLACE FUNCTION public.create_case_with_tasks(
  p_input jsonb,
...
REVOKE ALL ON FUNCTION public.create_case_with_tasks(jsonb, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.create_case_with_tasks(jsonb, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.create_case_with_tasks(jsonb, jsonb) TO authenticated;
